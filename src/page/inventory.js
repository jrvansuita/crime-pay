...
module.exports = class InventoryPage extends Page {
    ...
    routes() {
        ...
        this.get('/inventory-equip-form').then(({ player, req, res }) => {
            return this.weaponController.details(req.query.id, player).then(weapon => {
                res.render('partials/inventory-equip-form', { weapon, player })
            });
        });

        this.get('/inventory-sell-form').then(({ player, req, res }) => {
            return this.weaponController.details(req.query.id, player).then(weapon => {
                res.render('partials/inventory-sell-form', { weapon, player })
            });
        });
